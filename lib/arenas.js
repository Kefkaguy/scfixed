...
const DIFFICULTY_COLORS = {
  1: { color: "#44aaff", accentColor: "#aaddff", bg: "radial-gradient(ellipse 80% 60% at 50% 80%, #001133 0%, #070810 70%)" },
  2: { color: "#e8001a", accentColor: "#ff6644", bg: "radial-gradient(ellipse 80% 60% at 50% 80%, #2a0000 0%, #070810 70%)" },
  3: { color: "#9933cc", accentColor: "#cc66ff", bg: "radial-gradient(ellipse 80% 60% at 50% 80%, #1a0033 0%, #070810 70%)" },
  4: { color: "#008000", accentColor: "#d7ffd7", bg: "radial-gradient(ellipse 80% 60% at 50% 80%, #092300 0%, #070810 70%)" },
  5: { color: "#ff8800", accentColor: "#ffcc44", bg: "radial-gradient(ellipse 80% 60% at 50% 80%, #2a1000 0%, #070810 70%)" },
  6: { color: "#f0c020", accentColor: "#fff4aa", bg: "radial-gradient(ellipse 80% 60% at 50% 80%, #332200 0%, #070810 70%)" },
  7: { color: "#ff44aa", accentColor: "#ffc2e5", bg: "radial-gradient(ellipse 80% 60% at 50% 80%, #33001f 0%, #070810 70%)" },
  8: { color: "#00ddcc", accentColor: "#b9fff8", bg: "radial-gradient(ellipse 80% 60% at 50% 80%, #00302d 0%, #070810 70%)" },
  9: { color: "#c9ff3d", accentColor: "#f2ffb8", bg: "radial-gradient(ellipse 80% 60% at 50% 80%, #263300 0%, #070810 70%)" },
  10: { color: "#ffffff", accentColor: "#f0c020", bg: "radial-gradient(ellipse 80% 60% at 50% 80%, #333333 0%, #070810 70%)" },
};
...
export function buildArenaPresentation(arena) {
  ...
  return {
    ...
    tags: Array.isArray(arena?.tags) && arena.tags.length ? arena.tags : ["CUSTOM", "CLASS ARENA"],
...
export async function findArenaOwnerById(arenaId) {
  ...
  const usersCollection = await getUsersCollection();
  const userDocument = await usersCollection.findOne({ "customLevels.id": arenaId });
