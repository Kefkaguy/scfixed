...
function buildS3Key({ folder, namePrefix, fileName }) {
  const extension = String(fileName || "").includes(".") ? String(fileName).split(".").pop() : "bin";
  const safeFolder = sanitizeSegment(folder) || "uploads";
  const safeNamePrefix = sanitizeSegment(namePrefix) || "file";
  return `${safeFolder}/${safeNamePrefix}-${crypto.randomUUID()}.${extension}`;
}
...
export async function uploadFileToS3({ file, folder, namePrefix }) {
  ...
  const key = buildS3Key({ folder, namePrefix, fileName: file.name });
...
export async function createPresignedS3Upload({ fileName, contentType, folder, namePrefix }) {
  const { bucket } = getS3Config();
  const key = buildS3Key({ folder, namePrefix, fileName });
